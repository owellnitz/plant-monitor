...
one card per plant. A newly reporting sensor shows up on the **Sensors** page
(filter to *Unassigned*) until you create a plant for it (name, species,
location, sun exposure) and bind the sensor. Each plant's detail page has its
latest reading, a 7-day chart and the most recent readings, with edit/delete.
The **Sensors** page lists every reporting sensor with its firmware version,
