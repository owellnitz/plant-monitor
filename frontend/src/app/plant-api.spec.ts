...
describe('PlantApi', () => {
  ...
  it('fetches all sensors', () => {
    let sensors: SensorOverview[] | undefined;
    api.getSensors().subscribe((s) => (sensors = s));

    const payload: SensorOverview[] = [
      {
        deviceId: 'a1',
        raw: 1000,
        percent: 20,
  ...
  it('deletes a sensor, encoding the device id', () => {
    api.deleteSensor('a b/c').subscribe();
